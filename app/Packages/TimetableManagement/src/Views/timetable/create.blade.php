...
<nav style="font-size:.8125rem;color:#64748b;">
    ...
    <span style="color:#374151;font-weight:500;">Add Entry</span>
</nav>
@endsection
...
<div style="max-width:720px;">
    <div style="margin-bottom:1.5rem;">
        <h1 style="font-size:1.375rem;font-weight:700;color:#1e293b;margin:0;">Add Timetable Entry</h1>
        <p style="font-size:.8125rem;color:#64748b;margin:.25rem 0 0;">Schedule a class for a specific period and day</p>
    </div>
        ...
        <form method="POST" action="{{ route('timetable.store') }}">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;">
                ...
                <x-core-package::form.select name="section_id" label="Section" required>
                    ...
                    <option value="{{ $section->section_id }}" @selected(old('section_id') == $section->section_id)>{{ $section->section_name }}</option>
                    @endforeach
                </x-core-package::form.select>
                ...
                <x-core-package::form.select name="academic_year_id" label="Academic Year" required>
                    ...
                    <option value="{{ $year->academic_year_id }}" @selected(old('academic_year_id') == $year->academic_year_id)>{{ $year->year_range }}</option>
                    @endforeach
                </x-core-package::form.select>
                ...
                <x-core-package::form.select name="day_id" label="Day" required>
                    ...
                    @foreach($days as $day)
                    <option value="{{ $day->day_id }}" @selected(old('day_id') == $day->day_id)>{{ $day->day_name }}</option>
                    @endforeach
                </x-core-package::form.select>
                ...
                <x-core-package::form.select name="period_id" label="Period" required>
                    ...
                    <option value="{{ $period->period_id }}" @selected(old('period_id') == $period->period_id)>
                ...
                <x-core-package::form.select name="subject_id" label="Subject" required>
                    ...
                    <option value="{{ $subject->subject_id }}" @selected(old('subject_id') == $subject->subject_id)>{{ $subject->subject_name }}</option>
                    @endforeach
                </x-core-package::form.select>
                ...
                <x-core-package::form.select name="teacher_id" label="Teacher" required>
                    ...
                    <option value="{{ $teacher->staff_id }}" @selected(old('teacher_id') == $teacher->staff_id)>{{ $teacher->first_name }} {{ $teacher->last_name }}</option>
                    @endforeach
                </x-core-package::form.select>
                ...
                <x-core-package::form.select name="room_id" label="Room" required>
                    ...
                    <option value="{{ $room->room_id }}" @selected(old('room_id') == $room->room_id)>{{ $room->room_name }} ({{ $room->room_type }})</option>
                    @endforeach
                </x-core-package::form.select>
            </div>
            ...
            <div style="display:flex;gap:.75rem;margin-top:1.5rem;">
                <x-core-package::btn type="submit" color="primary">Save Entry</x-core-package::btn>
