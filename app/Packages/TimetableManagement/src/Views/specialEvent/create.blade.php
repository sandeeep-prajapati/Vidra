...
<nav style="font-size:.8125rem;color:#64748b;">
    ...
    <span style="color:#374151;font-weight:500;">Add Event</span>
</nav>
@endsection
...
<div style="max-width:640px;">
    <div style="margin-bottom:1.5rem;">
        <h1 style="font-size:1.375rem;font-weight:700;color:#1e293b;margin:0;">Add Special Event</h1>
        <p style="font-size:.8125rem;color:#64748b;margin:.25rem 0 0;">Schedule an exam, meeting, celebration or other event</p>
    </div>

    <x-core-package::card>
        <form method="POST" action="{{ route('specialEvent.store') }}">
            @csrf
            <div style="display:flex;flex-direction:column;gap:1.25rem;">
                <x-core-package::form.input name="event_name" label="Event Name" required
                    hint="e.g. Science Fair, Annual Exam, Staff Meeting"
                ...
                <x-core-package::form.select name="room_id" label="Room (optional)">
                    ...
                    <option value="{{ $room->room_id }}" @selected(old('room_id') == $room->room_id)>{{ $room->room_name }} ({{ $room->room_type }}, cap: {{ $room->capacity }})</option>
                    @endforeach
                </x-core-package::form.select>

                <x-core-package::form.textarea name="description" label="Description" rows="3"
                    hint="Event details or notes">{{ old('description') }}</x-core-package::form.textarea>
                ...
                <div style="display:flex;gap:.75rem;">
                    <x-core-package::btn type="submit" color="primary">Save Event</x-core-package::btn>
